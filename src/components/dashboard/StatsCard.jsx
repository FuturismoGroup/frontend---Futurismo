    ...
    <div className="card hover-lift group">
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-neutral-600 mb-1 truncate tracking-wide" title={title}>{title}</p>
          <p className="text-3xl font-bold text-neutral-900 mb-2 truncate gradient-text" title={String(value)}>{value}</p>

          {subtitle && (
            <p className="text-sm text-neutral-500 mb-1 truncate" title={subtitle}>{subtitle}</p>
          )}
          ...
          {trend && (
            <div className="flex items-center gap-2 min-w-0">
              ...
              {showTrendComparison && (
                <span className="text-sm text-neutral-500 truncate" title={t('dashboard.stats.vsPreviousMonth', 'vs mes anterior')}>
